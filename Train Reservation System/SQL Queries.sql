...
INSERT INTO Passenger(Username, EmailAddress) VALUES('+username+','+EmailAddress+');

/*Add School Info
Update the boolean flag to show that the user is a student if the email entered ends in ‘.edu’*/
UPDATE Passenger SET IsStudent = 1 WHERE Username='+username+';  
...
/*Users can Select how many Bags they will take & other Account Info:*/
...
AS StudentBagPrice FROM 
	(SELECT Ticket.ReservationID, Account.Username,Passenger.IsStudent,Ticket.DepartureDate, 
		...
		Ticket INNER JOIN Reservation ON Reservation.ReservationID = Ticket.ReservationID  INNER JOIN Account 
		ON Reservation.Username = Account.Username INNER JOIN Passenger ON Passenger.Username = Account.Username) AS P1 
...
AS Time,R2.StationName AS `Departs From`, R1.StationName AS `Arrives At`,R1.Class AS `Class`, R1.Price AS `Price`, R1.NumBags AS `#of Bags`, R1.PassengerName AS `Account Name` 
...
FROM 
	(SELECT P1.ReservationID, P1.NumUpdates, P1.IsStudent, P1.Price, P1.Username,IF(P1.IsStudent=1,P1.BagPrice*'+SDiscount+',P1.BagPrice) 
		...
		(SELECT Ticket.ReservationID, Account.Username,Passenger.IsStudent,Ticket.DepartureDate, Reservation.NumUpdates, 
			...
			FROM Ticket INNER JOIN Reservation ON Reservation.ReservationID = Ticket.ReservationID  INNER JOIN Account ON Reservation.Username = Account.Username INNER JOIN Passenger ON Passenger.Username = Account.Username)
	AS P1) AS P2 
...
SELECT P3.TotalCost AS `Total Cost`, CURDATE() AS `Cancel Date`,
	...
	(SELECT IF(P2.DepartureDate !=’DepartureDate’ , P2.StudentBagPrice + 50, StudentBagPrice) 
		...
		(SELECT P1.ReservationID, P1.IsStudent, P1.Price, P1.IsCancelled, P1.Username,
			...
			(SELECT Ticket.ReservationID, Account.Username,Passenger.IsStudent, Reservation.IsCancelled,Ticket.DepartureDate, 
...
IF( P3.IsCancelled!=0 OR DATE_ADD(CURDATE(), INTERVAL 1 DAY) <=P3.DepartureDate,P3.TotalCost,P3.TotalCost)))))+P3.TotalCost) AS UpdatedCost FROM
	(SELECT IF(P2.DepartureDate !=2016-06-02 , P2.StudentBagPrice + 50, StudentBagPrice) AS TotalCost, P2.Username, P2.ReservationID, P2.DepartureDate, P2.IsCancelled FROM 
		(SELECT P1.ReservationID, P1.IsStudent, P1.Price, P1.IsCancelled, P1.Username,
			IF(P1.IsStudent=1,P1.BagPrice*0.8,P1.BagPrice) AS StudentBagPrice, P1.BagPrice, P1.DepartureDate FROM 
			(SELECT Ticket.ReservationID, Account.Username,Passenger.IsStudent, Reservation.IsCancelled,Ticket.DepartureDate, 
				IF(4 < Ticket.NumBags+2, Ticket.Price + 30*(4-(Ticket.NumBags+2)), Ticket.Price) AS BagPrice,Ticket.Price,Ticket.NumBags FROM Ticket INNER JOIN Reservation ON Reservation.ReservationID = Ticket.ReservationID  INNER JOIN Account ON Reservation.Username = Account.Username INNER JOIN Passenger ON Passenger.Username = Account.Username) 
	AS P1) AS P2) AS P3 
